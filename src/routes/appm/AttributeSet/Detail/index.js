...
@connect(({ loading, proAttributeSet }) => ({
  proAttributeSet,
  loading: {
    detailLoading: loading.effects['proAttributeSet/fetchAttributeSetDetail'],
    // detailLineListLoading: loading.effects['proAttributeSet/queryDetailLineList'],
    fullTextSearchLoading: loading.effects['proAttributeSet/queryAttributeSetList'],
    saveDetailLoading: loading.effects['proAttributeSet/saveDetailData'],
  },
...
class Detail extends Component {
  ...
  fetchDetailList() {
    ...
    if (!isUndefined(params.id)) {
      // dispatch({
      //   type: 'proAttributeSet/queryDetailLineList',
      //   payload: {
      //     tenantId,
      //     page,
      ...
      // });
      dispatch({
        type: 'proAttributeSet/fetchAttributeSetDetail',
        payload: {
          tenantId,
          attributeSetId: params.id,
        },
      ...
      dispatch({
        type: 'proAttributeSet/updateState',
        payload: {
          detailList: [],
        },
      });
  ...
  handleSaveAttributeSet() {
    const {
      ...
      match,
      tenantId,
      proAttributeSet: { detail = {}, detailList = [] },
    } = this.props;
    ...
    if (flag) {
      dispatch({
        type: 'proAttributeSet/saveDetailData',
        payload: {
          tenantId,
          data: params,
        },
        ...
        if (res) {
          ...
          if (isUndefined(id)) {
            dispatch(
              routerRedux.push({
                pathname: `/appm/attribute-set/detail/${res[0].attributeSetId}`,
              })
            );
          } else {
  ...
  handleLinkToDetail(id) {
    this.props.dispatch(
      routerRedux.push({
        pathname: `/appm/attribute-set/detail/${id}`,
      })
    );
  }
  ...
  render() {
    ...
    const {
      ...
      match,
      tenantId,
      proAttributeSet: { detail = {}, detailList, list = [], pagination = {}, enumMap = {} },
    } = this.props;
    const { id } = match.params;
    ...
    return (
      <Fragment>
        <Header
          title={intl.get(`appm.proAttributeSet.view.message.title.detail`).d('属性组明细')}
          backPath="/appm/attribute-set/list"
        >
          <Button
            type="primary"
            icon="save"
