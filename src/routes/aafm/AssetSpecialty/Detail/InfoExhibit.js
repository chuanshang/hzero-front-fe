...
class InfoExhibit extends Component {
  ...
  render() {
    ...
    const prefix = 'aafm.assetSpecialty.view.message';
              ...
              <Col span={21}>
                <Row style={{ marginBottom: 8 }}>
                  <span style={{ fontSize: 18, fontWeight: 'bold' }}>
                    {dataSource.assetSpecialtyName}
                  </span>
                </Row>
                <Row>
                  <Col span={7}>
                    <Row>
                      <span>{intl.get(`${prefix}.assetSpecialtyName`).d('名称')}</span>
                    </Row>
          ...
          >
            ...
            >
                ...
                <Form>
                  ...
                  >
                    <Col {...FORM_COL_3_LAYOUT}>
                      <Form.Item
                        label={intl.get(`${modelPrompt}.assetSpecialtyName`).d('专业分类名称')}
                        {...EDIT_FORM_ITEM_LAYOUT}
                      >
                        {!isNew || editFlag ? (
                          getFieldDecorator('assetSpecialtyName', {
                            initialValue: dataSource.assetSpecialtyName,
                            rules: [
                              {
                                required: true,
                                message: intl.get('hzero.common.validation.notNull', {
                                  name: intl
                                    .get(`${modelPrompt}.assetSpecialtyName`)
                                    .d('专业分类名称'),
                                }),
                              },
                              {
              ...
              <Collapse.Panel
                ...
                header={
                  <Fragment>
                    <h3>{intl.get(`${prefix}.panel.B`).d('管理组织分配')}</h3>
                    <a>
