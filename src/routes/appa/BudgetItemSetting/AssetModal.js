...
class AssetModal extends PureComponent {
  ...
  handleRemoveColumn(columns) {
    const { productTypeFlag, assetSetFlag } = this.props;
  ...
  render() {
    const prefix = 'appa.budgetItemSetting.model.budgetItemSetting';
    const {
      dataSource,
      tenantId,
      modalVisible,
            ...
            <Form.Item>
              {record.$form.getFieldDecorator('productCategoryId', {
                ...
                <Lov
                  ...
                  // textValue={
                  //   !isUndefined(productCategoryName[record.itemToAssetId])
                  //     ? productCategoryName[record.itemToAssetId]
          ...
          ['create', 'update'].includes(record._status) ? (
            <Form.Item>
              {record.$form.getFieldDecorator('name', {
                initialValue: value,
              })(<Input disabled={!(productTypeFlag === 1 && assetSetFlag === 0)} />)}
            </Form.Item>
          ) : (
            value
          ),
      ...
      >
        <React.Fragment>
          <Row style={{ marginBottom: 10 }}>
            <Col>
              <Button type="primary" onClick={onNew} style={{ marginRight: 10 }}>
              ...
              <ExcelExport requestUrl={`${HALM_PPM}/v1/${tenantId}/budget-item/export`} />
            </Col>
          </Row>
          <Row>
            <EditTable
              bordered
              rowKey="itemToAssetId"
              columns={newColumns}
              loading={loading}
